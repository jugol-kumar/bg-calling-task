...
<template>
    <div class="w-full max-w-[calc(100%-13rem)] h-16 border-b backdrop-blur-xl transition-all ease-in-out fixed top-0 z-50" :class="isBlur ? 'bg-white/10' : 'bg-white'">
        <div class="px-8 flex w-full h-full justify-between items-center">
            <a href="/"
               target="_blank"
               class="block p-1 hover:bg-primary-500 my-4 border-primary-500/20 group dark:hover:text-white transition-all ease-in-out duration-300 rounded-lg">
                <Icon name="solar:earth-bold-duotone" size="10" class="text-primary-500 group-hover:text-white"/>
            </a>
            ...
            <div class="relative cursor-pointer">
                ...
                <div @click="toggleDropdown" ref="dropdownButton" id="dropdownDefaultButton">
                    <div class="flex gap-2 items-center  px-2 rounded-md">
                        ...
                        <div class="flex flex-col">
                            <p class="m-0 ps-0 font-semibold text-gray-600 text-sm capitalize">{{ authStore?.user?.name }}</p>
                            <small class="text-xs text-gray-400">{{ authStore?.user?.email }}</small>
                        </div>
                    </div>
                </div>
                ...
                <div v-show="isDropdownOpen" ref="dropdown" id="dropdown" class="z-10 bg-white divide-y divide-gray-100 rounded-lg overflow-hidden shadow w-44 dark:bg-gray-700 absolute mt-2">
                    ...
                    <ul class="text-sm text-gray-700 dark:text-gray-200" aria-labelledby="dropdownDefaultButton">
                        <li>
                            <RouterLink :to="{name:'Dashboard'}" class="flex items-center text-xs gap-3 px-4 py-2 hover:bg-gray-100 dark:hover:bg-gray-600 dark:hover:text-white">
                                <Icon name="solar:home-smile-angle-bold-duotone" size="20" />
                                <span>Dashboard</span>
                            </RouterLink>
                        </li>
                        <li>
                            <RouterLink :to="{name:'ProfileSetting'}" class="flex items-center text-xs gap-3 px-4 py-2 hover:bg-gray-100 dark:hover:bg-gray-600 dark:hover:text-white">
                        ...
                        <li class="border-t">
                            <a @click="authStore.logout"  class="flex items-center text-xs gap-3 px-4 py-2 hover:bg-gray-100 dark:hover:bg-gray-600 dark:hover:text-white">
